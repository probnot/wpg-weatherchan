...
def weather_page():
    ...
    global s8, s7, s6, s5, s4, s3, s2, s1, weathercol
    ...
    w_pad_day1 = ""
    for rng in range(35 * 2 - len_pad_day1):
    ...
    w_pad_day2 = ""
    for rng in range(35 * 2 - len_pad_day2):
    ...
    w_pad_day3 = ""
    for rng in range(35 * 2 - len_pad_day3):
    ...
    w_pad_day4 = ""
    for rng in range(35 * 2 - len_pad_day4):
        w_pad_day4 = w_pad_day4 + " "

    wsum_day5 = ec_en.daily_forecasts[5]["period"] + ". " + ec_en.daily_forecasts[5][
        "text_summary"]  # read in day4 text summary forecast
    ...
    if time_sec < 30:
        ...
        if (time_min % 2) == 0:  # screen 1 -- text summary forecasts
            ...
            s1 = "WINNIPEG - " + days[day] + ", " + months[month] + "." + ec_en.forecast_time[
                                                                          6:8] + "/" + ec_en.forecast_time[
                                                                                       :4] + " " + str(
                w_time) + ":" + ec_en.forecast_time[10:12] + " " + ampm
            s2 = " "
            if "value" in ec_en.conditions["wind_chill"]:
                s3 = "TEMP " + ec_en.conditions["temperature"]["value"] + "C (FEELS LIKE " + \
                     ec_en.conditions["wind_chill"]["value"] + "C)"
                s4 = ".. AND " + tendency.upper()
            else:
                s3 = "TEMPERATURE " + ec_en.conditions["temperature"]["value"] + "C" + " .. AND " + tendency.upper()
                s4 = ""
            s5 = "HIGH OF " + ec_en.conditions["high_temp"]["value"] + "C     " + "LOW OF " + \
                 ec_en.conditions["low_temp"]["value"] + "C"
            if "value" in ec_en.conditions["wind_dir"]:
                s6 = "WIND " + ec_en.conditions["wind_dir"]["value"] + " " + ec_en.conditions["wind_speed"][
                    "value"] + " KMH   VISIBILITY " + ec_en.conditions["visibility"]["value"] + "KM"
            else:
                if "value" in ec_en.conditions["visibility"]:
                    s6 = "VISIBILITY " + ec_en.conditions["visibility"]["value"] + "KM"
                else:
                    s6 = "HUMIDITY " + ec_en.conditions["humidity"]["value"] + "%    DEWPOINT " + \
                         ec_en.conditions["dewpoint"]["value"] + "C"
            s7 = "HUMIDITY " + ec_en.conditions["humidity"]["value"] + "%    DEWPOINT " + ec_en.conditions["dewpoint"][
                "value"] + "C"
...
wpg_desc = wpg.entries[0]["description"] + pad + wpg.entries[1]["description"] + pad + wpg.entries[2][
    "description"] + pad + wpg.entries[3]["description"] + pad + wpg.entries[4]["description"] + pad + wpg.entries[5][
               "description"] + pad + wpg.entries[6]["description"] + pad + wpg.entries[7]["description"]
